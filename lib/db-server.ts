...
const whatsAppChatSchema = new mongoose.Schema({
  ...
  unreadCount: { type: Number, default: 0 }, // Unread message count
  messageCount: { type: Number, default: 0 }, // Total messages in chat
  status: { type: String, enum: ["active", "archived", "closed"], default: "active" },
  window24HourEndsAt: { type: Date, default: null }, // When 24-hour window closes (for template messages)
  lastTemplateMessageAt: { type: Date, default: null }, // Track last template message time
  createdAt: { type: Date, default: Date.now },
  updatedAt: { type: Date, default: Date.now },
})
...
const whatsAppMessageSchema = new mongoose.Schema({
  ...
  sentBy: { type: mongoose.Schema.Types.ObjectId, ref: "User", default: null }, // Staff member who sent it
  sentByName: { type: String, default: "" }, // Name of staff who sent it
  window24HourValid: { type: Boolean, default: true }, // Was this sent within 24-hour window
  createdAt: { type: Date, default: Date.now },
})

whatsAppMessageSchema.index({ chatId: 1, createdAt: -1 })
whatsAppMessageSchema.index({ patientId: 1, createdAt: -1 })
whatsAppMessageSchema.index({ whatsappMessageId: 1 }, { sparse: true })
whatsAppMessageSchema.index({ status: 1 })

// Webhook logs for debugging WhatsApp webhooks
const whatsAppWebhookLogSchema = new mongoose.Schema({
  event: { type: String, required: true }, // message_sent, message_delivered, etc.
  patientPhone: { type: String, required: true },
  whatsappMessageId: { type: String, default: null },
  payload: { type: mongoose.Schema.Types.Mixed, required: true }, // Full webhook payload
  processed: { type: Boolean, default: true },
  processedAt: { type: Date, default: Date.now },
  createdAt: { type: Date, default: Date.now },
})

whatsAppWebhookLogSchema.index({ createdAt: -1 })
whatsAppWebhookLogSchema.index({ event: 1, patientPhone: 1 })

export const WhatsAppChat = mongoose.models.WhatsAppChat || mongoose.model("WhatsAppChat", whatsAppChatSchema)
export const WhatsAppMessage =
  mongoose.models.WhatsAppMessage || mongoose.model("WhatsAppMessage", whatsAppMessageSchema)
export const WhatsAppWebhookLog =
  mongoose.models.WhatsAppWebhookLog || mongoose.model("WhatsAppWebhookLog", whatsAppWebhookLogSchema)
