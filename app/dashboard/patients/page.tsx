...
export default function PatientsPage() {
  ...
  const handleExtraChargesRequest = async (e: React.FormEvent) => {
    ...
    if (!selectedPatient || !extraChargesForm.amount || !extraChargesForm.treatment) {
    ...
    try {
      const res = await fetch(`/api/billing/${selectedPatient._id}/extra-charges`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
                ...
                <form onSubmit={handleAddPatient} className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="sm:col-span-2">
                      <label className="block text-sm font-medium text-foreground mb-2">Patient Photo</label>
                        ...
                        {showPhotoUpload ? (
                          <div className="space-y-4">
                            ...
                            <PatientPhotoUpload
                              ...
                              isLoading={loading.addPatient || loading.updatePatient}
                            />
                            <button
                          ...
                          <label
                            ...
                            style={{
                              pointerEvents: loading.addPatient || loading.updatePatient ? "none" : "auto",
                              opacity: loading.addPatient || loading.updatePatient ? 0.5 : 1,
                            }}
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-muted border-b border-border">
                    <tr>
                      <th className="text-left px-4 sm:px-6 py-3 font-semibold text-muted-foreground">Photo</th>
                ...
                <div className="bg-card rounded-xl shadow-2xl border border-border p-6 max-w-md w-full">
                  ...
                  <form onSubmit={handleExtraChargesRequest} className="space-y-4">
                    ...
                    <div className="flex gap-3 pt-4">
                      ...
                      >
                        {loadingExtraCharges && <Loader2 className="w-4 h-4 animate-spin" />}
                        Send Request
                      </button>
                    </div>
                  </form>
