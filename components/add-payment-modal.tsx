...
export function AddPaymentModal({ patientId, remainingBalance, isOpen, onClose, onSuccess }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/billing/${patientId}/payment`, {
        method: "POST",
        headers: {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md w-[95vw] sm:w-full p-4 sm:p-6 max-h-[85vh] overflow-y-auto">
        <DialogHeader className="mb-6">
          ...
          <DialogDescription className="sr-only">Record a new payment for the patient</DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="bg-muted p-4 rounded-lg mb-6">
            ...
            <p className="text-2xl font-bold text-destructive">${remainingBalance.toFixed(2)}</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g., Payment for visit..."
